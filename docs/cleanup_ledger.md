...
| Manual expensive/debug routes | `/api/digest/generate*`, `/api/onboard/backfill-curricula`, `/api/export/features`, `/api/cron/generate-digests` were callable in production with ordinary auth or no secret fallback in some cases | Phase 0c requires expensive dev/admin routes to be gated beyond UI | Cleaned in Phase 0c | Production now hides these routes unless `CRON_SECRET` bearer is supplied; local dev remains open |
...
| Onboarding browser storage | `/onboard` used `sessionStorage` to preserve chat/intent during Gmail redirect | Phase 0c requires server-owned onboarding truth | Cleaned in Phase 0c | Removed browser-storage recovery; chat prompt context now reads persisted server messages |
